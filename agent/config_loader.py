...
_ENVIRONMENT_VARIABLE_PATTERN = re.compile(r"\$\{(\w+)(?::-(.*?))?\}")

_CONFIG_DIRECTORY = Path(__file__).resolve().parent.parent / "config"
_SHARED_PATH = _CONFIG_DIRECTORY / "shared.yaml"
_PROVIDER_FILES = {
    "openai": _CONFIG_DIRECTORY / "openai.yaml",
    "ollama": _CONFIG_DIRECTORY / "ollama.yaml",
}
...
def _resolve_env_vars(value: Any) -> Any:
    ...
    if isinstance(value, str):
        ...
        return _ENVIRONMENT_VARIABLE_PATTERN.sub(_replacer, value)
