...
from agent.tools.unit_converter import UnitConverterTool
...
async def test_km_to_miles(converter):
    result = await converter.call({"value": 10, "from_unit": "km", "to_unit": "miles"})
...
async def test_celsius_to_fahrenheit(converter):
    result = await converter.call({"value": 100, "from_unit": "celsius", "to_unit": "fahrenheit"})
    assert abs(result["result"] - 212.0) < 0.01


@pytest.mark.asyncio
async def test_kg_to_lb(converter):
    result = await converter.call({"value": 1, "from_unit": "kg", "to_unit": "lb"})
    assert abs(result["result"] - 2.2046) < 0.01


@pytest.mark.asyncio
async def test_same_unit_identity(converter):
    result = await converter.call({"value": 42, "from_unit": "km", "to_unit": "km"})
...
async def test_fahrenheit_to_kelvin(converter):
    result = await converter.call({"value": 32, "from_unit": "fahrenheit", "to_unit": "kelvin"})
...
async def test_unsupported_conversion_raises(converter):
    with pytest.raises(ValueError, match="Unsupported conversion"):
        await converter.call({"value": 1, "from_unit": "km", "to_unit": "kg"})
